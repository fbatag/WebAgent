...
gcloud run deploy $SERVICE_NAME_UI --source ./UI --region=$REGION \
   --service-account=$SERVICE_ACCOUNT@$PROJECT_ID.iam.gserviceaccount.com \
   --project=$PROJECT_ID  --allow-unauthenticated  
   --min-instances=1 --timeout=5m --cpu-throttling 
   --verbosity=debug 
   --quiet

export REGION=us-central1
gcloud run deploy $SERVICE_NAME_UI-us --source ./UI --region=$REGION \
   --service-account=$SERVICE_ACCOUNT@$PROJECT_ID.iam.gserviceaccount.com \
   --project=$PROJECT_ID  --allow-unauthenticated  
